...
val simulationResultValueComparator = Comparator<SimulationResultValue> { a, b ->
    sequence {
        a.tags.zip(b.tags).forEach { (aTags, bTags) ->
            yield(aTags.compareTo(bTags))
        }
        yield(-a.tags.size.compareTo(b.tags.size))
    }.firstOrNull { it != 0 } ?: simulationResultTypeComparator.compare(a.result, b.result)
}

val simulationResultTypeComparator = Comparator<SimulationResultType> { a, b ->
    when {

...
fun codeMirrorConfig(parent: HTMLElement) = json(
    "parent" to parent,
    "extensions" to arrayOf(
        ...
        keymap.of(
            defaultKeymap + searchKeymap + historyKeymap + foldKeymap + lintKeymap + arrayOf(
                indentWithTab,
                jsObject {
                    win = "Mod-Shift-z"
                    run = redo
...
val movesetLanguage = run {
    ...
    StreamLanguage.define(
        json(
            ...
            "languageData" to json(
                "commentTokens" to json(
                    "line" to "#"
                )
            )
        )
...
val movesetFoldService = run {
    ...
    val service = service@{ state: dynamic, lineStart: dynamic, lineEnd: dynamic ->

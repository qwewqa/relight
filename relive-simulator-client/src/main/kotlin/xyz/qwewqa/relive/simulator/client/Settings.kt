...
data class UserData(
  ...
  @Transient val presetData = SyncMap(presets)

  @Transient val setupData = SyncMap(setups)
...
class SyncMap<T : Any>(
  ...
  override fun remove(key: String): T? {
    val now = Date.now().toLong()
    return map[key].also { map[key] = SyncData(now, null) }?.data
  }
...
fun <T : Any> MutableMap<String, SyncData<T>>.update(other: Map<String, SyncData<T>>) {
  other.forEach { (k, v) -> this[k] = this[k].update(v) }
}

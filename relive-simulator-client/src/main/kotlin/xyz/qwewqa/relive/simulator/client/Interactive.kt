...
private fun updateStageEffects(status: InteractiveQueueStatus?) {
  val teamStageEffectsContainer =
      document.getElementById("interactive-team-stage-effects-container") as HTMLElement
  val enemyStageEffectsContainer =
      document.getElementById("interactive-enemy-stage-effects-container") as HTMLElement
...
private fun updateActs(simulation: InteractiveSimulation, status: InteractiveQueueStatus?) {
  ...
  val interactiveShortcutsContainer =
      document.getElementById("interactive-shortcuts-container") as HTMLElement
            ...
            if (card == null) {
              ...
              when (card.status) {
                ActionStatus.READY -> onClickFunction = {
                      GlobalScope.launch { simulation.sendCommand("queue ${card.handIndex}") }
                    }
                ActionStatus.QUEUED -> onClickFunction = {
                      GlobalScope.launch { simulation.sendCommand("unqueue ${card.handIndex}") }
                    }
                else -> {}
              }
              when (card.status) {
              ...
              classes =
                ...
                if (card == null ||
                  ...
                  onClickFunction =
                      if (status.hold != null) {
                        {
                          GlobalScope.launch { simulation.sendCommand("discard ${card.handIndex}") }
                        }
                      } else {
                        { GlobalScope.launch { simulation.sendCommand("hold ${card.handIndex}") } }
                      }
                }
              }
        }
        ...
        div("d-flex flex-column") {
          button(
              classes =
                ...
                when {
                  status.canClimax -> {
                    div("interactive-climax-button-text-container") {
                      div("interactive-climax-button-text font-monospace") { +"CA" }
                    }
                  }
                ...
                onClickFunction = { GlobalScope.launch { simulation.sendCommand("climax") } }
          ...
          button(classes = "btn btn-sm btn-dark interactive-act-button") {
            ...
            onClickFunction = {
              if (status.runState == InteractiveRunState.QUEUE) {
                GlobalScope.launch { simulation.sendCommand("auto_queue") }
              }
            }
          }
        }
    ...
    status.runState == InteractiveRunState.SETUP -> {
      ...
      interactiveShortcutsContainer.append {
        button(classes = "btn btn-outline-secondary interactive-ui-shortcut-button") {
          role = "button"
          i("bi bi-shuffle")
          onClickFunction = { GlobalScope.launch { simulation.sendCommand("seed") } }
        }
      }
    }
  }
...
private fun updateMemoirs(simulation: InteractiveSimulation, status: InteractiveQueueStatus?) {
  val memoirCostText =
      document.getElementById("interactive-available-memoir-cost-text") as HTMLSpanElement
  val remainingCost = (status?.cutinCostMax ?: 0) - (status?.cutinCostUsed ?: 0)
  memoirCostText.textContent = "$remainingCost"

  val interactiveMemoirsContainer =
      document.getElementById("interactive-memoirs-container") as HTMLElement
      ...
      div(
          "interactive-memoir-image-container ${
            ...
            when (cutin.status) {
              ActionStatus.READY -> {
                onClickFunction = {
                  GlobalScope.launch { simulation.sendCommand("cutin ${cutin.actorName}") }
                }
              }
              ActionStatus.QUEUED -> {
                onClickFunction = {
              ...
              ActionStatus.COOLDOWN -> {
                span("interactive-memoir-cooldown-text") { +"${cutin.remainingColdown}t" }
              }
              ActionStatus.TOO_EXPENSIVE -> {
                span("interactive-memoir-too-expensive-text") { +"C" }
              }
              ActionStatus.HOLDER_EXITED -> {
                span("interactive-memoir-holder-exited-text") { +"X" }
              }
              else -> {}
            }
          }
  ...
  interactiveMemoirsContainer.append { status.cutins.forEach { memoir(it) } }
}

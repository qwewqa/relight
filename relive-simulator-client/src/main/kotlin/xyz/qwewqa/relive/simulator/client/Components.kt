...
class SongEffect(val element: Element) {
  ...
  private val conditions =
      element.getElementsByClassName("song-effect-condition").multiple<HTMLSelectElement>().map {
        MultipleSelect(it, true)
      }
  private val description =
      element.getElementsByClassName("song-effect-description").single<HTMLElement>()

  fun registerListeners() {
    type.element.addEventListener("change", { update() })
    value.element.addEventListener("change", { update() })
    conditions.forEach { it.element.addEventListener("change", { update() }) }
    element
        .getElementsByClassName("clear-select-button")
  ...
  var parameters: SongEffectParameter?
    ...
    set(param) {
      ...
      conditions.zip(param?.conditions ?: emptyList()).forEach { (a, b) -> a.value = b }

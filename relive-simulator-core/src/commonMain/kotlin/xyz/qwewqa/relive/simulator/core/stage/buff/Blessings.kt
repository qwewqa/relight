...
fun Actor.activateBlessings() = context.targetSelf().act {
    buffs.consumeOnce(Buffs.BlessingHpRecovery) { value ->
        heal(percent = value)
    }
    ...
    buffs.consumeOnce(Buffs.BlessingApDown2) { _ ->
        applyTimedBuff(ApDown2Buff, 0, 1)
    }
    buffs.consumeOnce(Buffs.DisasterBrillianceReduction) { value ->
        removeBrilliance(value)
    }
    buffs.consumeOnce(Buffs.DisasterDaze) { value ->
        applyCountableBuff(Buffs.DazeBuff, count = value)
    }
    buffs.consumeOnce(Buffs.DisasterApUp) { _ ->
        applyTimedBuff(ApUpBuff, 0, 1)
    }
}

...
/**
 * A strategy dictates what queue a team performs each turn. Much of validation is left to the
 * strategy itself, such as whether an act or cutin may be played or not.
 */
interface Strategy {
...
class ActionTile(val actor: Actor, val apCost: Int, val actData: ActData) : QueueTile() {
  override val agility
    get() = actor.mod { +agility }
...
data class BoundCutin(val actor: Actor, val data: CutinData) {
  ...
  override fun toString() =
      "@(memoir:${actor.memoir?.id})[${actor.name} (${actor.dress.name})]:[${actor.memoir?.name}]"
}

object EmptyStrategy : Strategy {
  override fun nextQueue(stage: Stage, team: Team, enemy: Team) =
      QueueResult(mutableListOf(), false)
}

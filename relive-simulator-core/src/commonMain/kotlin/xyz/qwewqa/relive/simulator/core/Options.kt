...
import xyz.qwewqa.relive.simulator.common.getMusicCoverArtUrl
                    ...
                    descriptionIcons =
                        listOfNotNull(
                            dress.character.school.id?.let {
                              "${ASSET_BASE_URL}res/ui/images/icon_colored/60x56_school$it.png"
                            },
                            dress.attribute.id?.let {
                              "${ASSET_BASE_URL}res/ui/images/icon_colored/56x56_attribute$it.png"
                            },
                            dress.damageType.id?.let {
                              "${ASSET_BASE_URL}res/ui/images/icon_colored/54x54_attack_type$it.png"
                            },
                            dress.position.id?.let {
                              "${ASSET_BASE_URL}res/ui/images/icon_colored/54x54_position$it.png"
                            },
                        ),
                    description = localeIds.associateWith { "c${dress.cost}" },
                    tags = tags,
                    imagePath =
                        "${ASSET_BASE_URL}res/item_root/large/1_${dress.id}.png"
                            .takeIf { dress.id > 0 },
                    data =
                        DressData(
                            id = dress.id,
      ...
      memoirs =
          Memoirs.map { (id, memoir) ->
                DataSimulationOption(
                    ...
                    imagePath =
                        if (memoir.id > 0) {
                          "${ASSET_BASE_URL}res/item_root/large/2_${memoir.id}.png"
                        } else {
                          "${ASSET_BASE_URL}res/ui/images/common/plate_unselected_3.png"
                        },
                    data =
                        MemoirData(
                            id = memoir.id,
      ...
      songs =
          Songs.map { (id, song) ->
            ...
            DataSimulationOption(
                ...
                descriptionIcons =
                    awakenSkills.filter { it.iconId != null }.map { getSkillIconUrl(it.iconId) },
                tags =
                    localeIds.associateWith { locale ->
                      awakenSkills.map { it.names.getLocalizedString(locale) }
                    },
                imagePath = getMusicCoverArtUrl(id).takeIf { id >= 0 },
                data =
                    SongData(
                        id = song.id,
          ...
          Songs.filter { (_, song) -> song.awakenExtraSkill != null }
              .map { (id, song) ->
                ...
                DataSimulationOption(
                    ...
                    descriptionIcons = listOf(getSkillIconUrl(skill.iconId)),
                    imagePath = getMusicCoverArtUrl(id),
      ...
      songEffects =
          BasicSongEffects.map { (id, effect) ->
            DataSimulationOption(
                id = "$id",
                name = effect.names,
                imagePath = getSkillIconUrl(effect.iconId).takeIf { effect.iconId != null },
                data =
                    SongEffectData(
                        id = id,
                    ),
      ...
      awakeningSongEffects =
          AwakeningSongEffects.map { (id, effect) ->
            DataSimulationOption(
                id = "$id",
                name = effect.names,
                imagePath = getSkillIconUrl(effect.iconId),
                data =
      ...
      bosses =
          bossLoadouts.map { (k, v) ->
            ...
            DataSimulationOption(
                ...
                imagePath = v.info?.iconId?.let { "${ASSET_BASE_URL}res/icon/enemy/$it.png" },
      ...
      remakeSkills =
          remakeSkills.values.map { skill ->
            ...
            DataSimulationOption(
                ...
                imagePath = getSkillIconUrl(skill.icon),
                data =
                    RemakeSkillData(
      ...
      accessories =
          Accessories.map { (id, accessory) ->
            DataSimulationOption(
                ...
                imagePath =
                    if (accessory.iconId == 0)
                        "${ASSET_BASE_URL}res/ui/images/common/plate_unselected_6.png"
                    else "${ASSET_BASE_URL}res/item_root/medium/38_${accessory.iconId}.png",
                data =
                    AccessoryData(
                        id = accessory.id,

...
fun Application.configureRouting() {
  routing {
    ...
    val options by lazy {
      ...
      val remakeSkillNames =
          getLocalizationConfig("remakeSkill.yaml").associate { it.id to it.name }
          ...
          getLocalizationConfig("dress.yaml", playerDresses.keys).map { option ->
            ...
            val tags =
                (option.tags ?: locales.keys.associateWith { emptyList() }).mapValues {
                    (locale, tags) ->
                  val characterTag =
                      commonTextById["character-${dress.character.name.lowercase()}"]?.get(locale)
                  val schoolTag =
                      commonTextById["school-${dress.character.school.name.lowercase()}"]?.get(
                          locale)
                  val attributeTag =
                      commonTextById["attribute-${dress.attribute.name.lowercase()}"]?.get(locale)
                  val damageTypeTag =
                      commonTextById["damage-type-${dress.damageType.name.lowercase()}"]?.get(
                          locale)
                  val positionTag =
                      commonTextById["position-${dress.position.name.lowercase()}"]?.get(locale)
                  tags +
                      listOfNotNull(
                          characterTag,
                          schoolTag,
    ...
    get("/options") { call.respond(options) }
